...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (user) {
            ...
            var table = "<h1 style='text-align:center;text-decoration:underline;'>employee data </h1><table style='border:2px solid black;padding:10px;margin:0 auto;'><tr><th style='text-align:left;border-bottom:2px solid black;padding:5px;margin:5px;'>username</th><th style='text-align:left;border-bottom:2px solid black;padding:5px;margin:5px;'>email</th><th style='text-align:left;border-bottom:2px solid black;padding:5px;margin:5px;'>password</th></tr>";
            for(let data of emp_datas){
                table = table + `<tr><td style='border-bottom:2px solid black;padding:5px;margin:5px;'>${data.username}</td><td style='border-bottom:2px solid black;padding:5px;margin:5px;'>${data.email}</td><td style='border-bottom:2px solid black;padding:5px;margin:5px;'>${data.password}</td></tr>`;
            }
            table = table + "</table>";
